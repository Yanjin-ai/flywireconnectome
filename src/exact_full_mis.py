...
def main():
    ...
    t0 = time.time()
    ub = clique_cover_ub(D)
    print(f"  Clique-cover UPPER bound: N <= {ub}  ({time.time()-t0:.1f}s)")
    ...
    out = {
        ...
        "upper_bound": {"N": ub, "method": "greedy clique cover"},
        "cbc_crosscheck": cbc,
        ...
        "note": ("Full-graph exact MIS does not close in reasonable time (one "
                 ...
                 "solution. N=105 is verified feasible; this brackets the true "
                 "optimum."),
    }
