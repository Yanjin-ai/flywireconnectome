...
- **Upper bound — N ≤ 245**, from a greedy **clique cover** of D taken as the *minimum* over 40 randomised vertex orderings (any independent set meets each clique at most once, so the number of cliques covering V bounds the independence number; randomised restarts tighten a single degree-ordered pass from 272 down to 245).

So the certified statement is **105 ≤ N* ≤ 245**, with N = 105 a verified-feasible solution and the subgraph experiments (§3.2: ~97% of exact on hard samples) indicating the truth is much nearer the lower end than the clique-cover bound suggests. The clique-cover upper bound is still loose; the principled tightening is the Lovász ϑ SDP bound (or a clique-cut ILP), which needs an SDP solver (cvxpy/SCS) not currently in the dependency set — a clean, self-contained future improvement.
...
- **N is a verified-feasible lower bound, bracketed by a loose upper bound.** N = 105 is a *verified* common subgraph (0 internal disagreement; §3.6), not merely a heuristic output. The exact optimum is certified to lie in **[105, 245]** (greedy clique-cover upper bound). The honest greedy optimality gap is **2.67%** under hard (disagreement-ego) subgraph sampling — higher than the 1.15% under uniform sampling, which is optimistic (§3.2, §3.4). The 100-seed variance of ±2.2 (§4.1) is independent evidence of stability. A *certified-optimal* proof on the full 987-node single dense component remains intractable; tightening the upper bound (Lovász ϑ / clique-cut ILP) is open.
